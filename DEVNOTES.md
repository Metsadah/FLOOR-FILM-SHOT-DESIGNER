...
now a separate SPM target without Capacitor) + photos from a director's
viewfinder (sensor + focal → HFOV → the tightest iPhone lens that covers
it, frame overlay, centre crop on capture) + A4 PDF (cover, plan pages
with dimensions and a 1 m bar, photo pages) + optional Floorboard link
(supabase-swift: magic link → floorboardscout://login; kv rows
sd:room:/sd:location:, photos in Storage bucket 'scout' — see ROOMS.md).
Offline first: everything in Documents, dirty sets pushed on sync.
Project file is generated by scout/gen-project.js (objectVersion 77,
synchronized folder) — no Xcode GUI needed; scout/install.sh builds and
installs. Supabase: bucket + policies applied by migration
scout_photos_bucket; the redirect URL must be added in the dashboard.
...
strings, iPad shell appName/CFBundleDisplayName). New mark: rounded
board, three floor lines, one coral dot (index.html inline SVG; icons/
regenerated by scratchpad mkicons.js — a dependency-free SDF rasterizer
that writes PNGs via zlib; the iOS AppIcon-512@2x.png is the maskable
variant). Floors in #tabbar, each button = icon + level pill + clear
name: Ground·Mood, 1st·Script (AV script is a *function* there, no floor
of its own), 2nd·Shot designer, 3rd·Budget (new), 4th·Production.
Budget (js/09-budget.js): project.budget = {currency, target:{pre,prod,
post}, items:[{id,cat,name,phase,qty,unit,rate,note,personId?}]}. DOM
page #tabBudget inside #main (body.tab-budget hides the three columns
and the scene title). Rows import from the People registry (crew/cast,
role stays attached) and from gearListGroups(); CSV export (;-separated,
BOM, nl-NL currency). Phase cards colour red when over target.
Production floor: "Documents" section on top of the library —
collectDocCards() finds every script / AV script card (script board,
moodboard, prod board, nested sub-boards, scene boards) with PDF / .docx
/ Open buttons, plus the budget CSV. The 'Production' info tile is now
Production-floor only (the other production cards already were).
Not in this release (next): the shot list from the Shot designer with
shooting order, breaks, moves and shoot days; templates per project type.
