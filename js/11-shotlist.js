    ...
    for(const {su, objs} of pools){
      for(const ob of objs){
        ...
        out.push({key:s.id + '|' + ob.id, sceneId:s.id, camId:ob.id, si,
          ...
          framing:ob.framing || '', lens:ob.lens ? lensLabel(ob) + (typeof sensorShort === 'function' && sensorShort(ob.sensor) ? ' (' + sensorShort(ob.sensor) + ')' : '') : '', support:ob.support || '', setup:suName, camName,
